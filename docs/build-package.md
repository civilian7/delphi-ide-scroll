...
- 패키지는 `rtl`, `vcl` 만 require 하므로 추가 컴포넌트 불필요
...
4. 제거하려면 같은 화면에서 패키지를 선택 후 **Remove**.
...
- **"does not contain any registered components" 경고** — 정상입니다. 이 패키지는 컴포넌트를 등록하지 않고 로드 시 훅만 활성화합니다.
